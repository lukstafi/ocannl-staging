...
module Impl : Ir.Backend_impl.Lowered_backend = struct
  ...
  let%diagn2_sexp hip_to_code ~name hip_src =
    ...
    in
    let options =
      hip_include_opt @ ("-ffast-math" :: (if Utils.with_runtime_debug () then [ "-g" ] else []))
    in
  ...
  struct
    ...
    (* Tile-MMA cooperative emission is not implemented for HIP (rocWMMA / builtin WMMA intrinsics
        ...
        List.map kparams ~f:(function
          ...
          | _name, Static_idx s ->
              ...
              if !i < 0 then
                ...
                @@ Utils.User_error
                     [%string
                       "hip: static index %{Indexing.symbol_ident s.static_symbol} is negative: \
                        %{!i#Int}"];
              Option.iter s.static_range ~f:(fun upto ->
                  if !i >= upto then
                    ...
                    @@ Utils.User_error
                         [%string
                           "hip: static index %{Indexing.symbol_ident s.static_symbol} is too big: \
                            %{upto#Int}"]);
              S.Int !i
      ...
      lazy
        (let attrs =
         ...
         let min_over f = Array.map attrs ~f |> Array.min_elt ~compare:Int.compare in
         {
           ...
           (* No cooperative tile-MMA emission on HIP yet ([mma_syntax = None]). *)
           mma = None;
         })
    in
