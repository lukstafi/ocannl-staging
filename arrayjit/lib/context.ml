...
(* The pre-dispatch validation of {!run}, callable on its own (gh-ocannl-550): everything here
   ...
   so an unattributed failure at [Launch] means dispatch was attempted — which is what makes
   condemning the lineage there sound, and what keeps a mere unsatisfied dependency or an
   out-of-range binding from condemning it (gh-ocannl-564). *)
