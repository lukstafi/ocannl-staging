...
- **A test that decides its own verdict reports it through `Verdict`** (`test/support/verdict.ml`; add `arrayjit.verdict` to the stanza's `(libraries ...)` — the bare private name `verdict` links fine in a dev build but is unresolvable under the `dune build -p <pkg>` that each `.opam`'s with-test command runs, which disables the stanza owning it): `Verdict.p "claim" b`, `pass_fail`, `fail`, and `skipped ~backend` for a leg this backend cannot evaluate. A skip whose gate belongs to the host or configuration rather than the selected backend (a compiler target, preprocessing flag or filesystem feature) passes ``~aggregation:`Environment``, so its human announcement remains but the backend sweep does not report it as universal backend non-coverage. A claim quantified over a collection goes through `p_all`/`p_none`/`p_exists`/`p_empty ~over`, never `p` applied to a `List.for_all` — vacuously true on an empty collection and indistinguishable in the golden (gh-ocannl-729); the unguarded spelling stays only where emptiness is the passing case, said so at the site. A failed claim exits the process 1, which is what keeps a failure from being `dune promote`d into the golden (gh-ocannl-601). Phrase every claim so `true` is the passing reading — in a golden a blessed regression and a designed negative are the same line — and keep purely descriptive output on plain `printf`, which is not an assertion channel. `test/operations/verdict_ratchet` enforces the claim shapes (a descriptive print that trips it takes a named exemption there); `Ll_test.p` is `Verdict.p`
