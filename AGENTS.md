...
- **A GPU suite runs width-capped**: `-j 2` on a WSL2 box (`rog-nv-wsl`, `minix-amd-wsl`); on a native boot, per slot, `-j 4` for hip on minix, `-j 8` for hip on tuf and for every backend on rog (gh-ocannl-1033; cc too, gh-ocannl-1065). At dune's default width the device bridge or copy-engine pool overflows and the suite comes back red in exactly the stanzas a real backend regression lands in. `tools/test-run.sh run` injects the cap from a GPU `OCANNL_BACKEND` in the environment (on the fleet's native rog from any); with the backend only in `ocannl_config`, pass `-j` yourself. On a fleet box `run`/`start` takes the fleet's run-time slot itself, `--cpu` only when every test config resolves a CPU backend and the targets reach no stanza naming a GPU backend; `SLOT REFUSED` (exit 75) is the slot's verdict, not the suite's (gh-ocannl-1004)
