...
- **Rebase onto fresh `origin/master` before opening the PR and again before merging**: GitHub builds the pull request's MERGE commit, so a check that scans the whole repository can be red on the tree CI builds while green on your branch — `docs/agent-notes/build-and-test.md` explains the mechanism and how to validate against the merged tree
