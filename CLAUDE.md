...
- A **PR accomplishes a goal**: one thing that is true about the system afterwards and was not before, stated in its title. Scope it generously — carry the goal to its natural completion (the change, the tests that pin it, the docs it justifies, the follow-on cleanups it exposes) rather than stopping at the smallest reviewable increment and opening three PRs. Velocity comes from finishing a goal in one pass; a stack of half-goals costs more review in total, not less, and no single one of them can be judged on its merits
- The bound on that is **one goal, not one size**. A large PR serving a single goal is fine; a PR accomplishing two unrelated goals should split however small each is, because the reviewer has to hold two independent models at once and neither can be reverted alone. An unrelated problem noticed along the way belongs in its own issue or PR — note it, don't absorb it
...
- A goal usually takes several moves, so a series of topical commits is the norm — merged with a merge commit that preserves the series, not squashed. Still, let the work decide the count: don't pad a series to reach a number, and don't collapse genuinely separate moves to avoid one
