...
- Instead, run the one test by its own alias: `dune build @test/operations/runtest-<name>` (dune >= 3.20 generates `runtest-<name>` per `(test)`/`(tests)` name and per inline-test library). It runs the exe with dune's cwd AND applies the `.expected` diff, so a changed output fails the build instead of sitting unread in a file, and `dune promote` accepts it; a misspelled name is an error ("Alias ... is empty"), not a silent no-op. The run still leaves `_build/default/<dir>/<name>.exe.output` to inspect
- A test written as an `(executable)` plus a `(rule)` that diffs — the `@slow` runners, the scanning tests — has no such alias. Build the rule's target (`dune build test/operations/<name>.actual`) and diff it yourself, then `dune runtest <dir>` to promote. For `bin/` executables (same cwd trap), pin `OCANNL_BACKEND=...` explicitly
- Pinning a variable on such a probe (`OCANNL_BACKEND=cuda dune build @test/operations/runtest-<name>`) reaches the run only for variables the stanza DECLARES: dune tracks no environment variable it was not told about, so an undeclared one leaves the target up to date and the previous run's result in place. Every test stanza declares `ocannl_backend` and `OCANNL_BACKEND`; for any other key add BOTH spellings to that stanza's `(deps ...)` first — `test/operations/env_var_deps` checks the pairing (gh-ocannl-628)
...
- **Test Placement Guidelines**:
  ...
  * When adding a test, update the corresponding test stanza — including `ocannl_config` in its `(deps ...)`, plus `(env_var OCANNL_BACKEND)` if the test picks a backend rather than naming one
