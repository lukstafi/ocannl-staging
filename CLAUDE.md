...
- Never judge `dune runtest` through a pipe: `... 2>&1 | tail -3 && echo OK` reports the status of `tail`, not dune (no pipefail), and promotion-diff failures look like normal chatter. To capture output while preserving Dune's status, run `( dune runtest; status=$?; printf 'exit: %s\n' "$status"; exit "$status" ) > /tmp/suite.log 2>&1`; append `& pid=$!` for a background run. Both `wait "$pid"` and the logged `exit: N` sentinel then report Dune's status; grep the log for `^File .*expected` to list promotion diffs
