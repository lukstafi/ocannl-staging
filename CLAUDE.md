...
- Keep library sources unchanged while Dune is running; edits invalidate in-flight rules and can repeat expensive work. For a background run, record its exact PID and exit status; do not poll with `pgrep -f`, which can match the waiter itself. `pgrep -x dune` is no safer: it matches any process *named* `dune`, including the editor's long-lived `dune ocaml-merlin --no-print-directory` daemons, which never exit — so `while pgrep -x dune; do sleep; done` spins forever (staging PR #337 accumulated ten such waiter shells, the oldest running over two hours, each waking a sleep every 25s). Wait on the PID captured when the job was launched (`kill -0 "$pid"`), or poll the log for the `exit:` sentinel the wrapper above already writes
