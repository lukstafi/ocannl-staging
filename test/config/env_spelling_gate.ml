(* gh-ocannl-652, Codex P1 on PR #389 (rounds 1 and 2): the ambient check a cached suite cannot
   hide. Copied into every test directory that runs OCANNL executables -- see the `(copy_files)`
   and `(test)` pair in each of their dune files, and `env_var_deps`, which fails if a directory
   with runtest actions has no gate.
   ...
   The `(universe)` dependency says "the state of the world decides this action", so dune reruns
   the gate on every invocation, and running it is the whole check: the check lives in `Utils`'
   own initialization, so a rejected spelling kills this process before `main` and the suite goes
   red naming the variable.
   ...
   itself, and enumerating it would be 112 keys per directory, a consistency check to keep each
   list whole, and an exemption in `env_var_deps` for declaring names nothing reads -- to buy a
   narrower guarantee than the one line gives. It also covers what the old per-stanza declarations
   never did: those named `backend` (218 stanzas) and eight other keys once each, so a lowercase
