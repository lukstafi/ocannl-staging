...
let check_stage1_defer name rho constrs =
  match Row.solve_inequalities ~stage:Stage1 constrs Row.empty_env with
  | _, env ->
      Stdio.printf "%s: no error, rho solved in stage1: %b\n" name
        (Option.is_some (Row.get_row_from_env env rho))
  | exception Row.Shape_error (msg, _) -> Stdio.printf "%s: Shape_error: %s\n" name msg

let prop_2_9_stage1_witness ~eq_first =
  let rho = Row.get_row_var () in
  let rho_row = Row.get_row_for_var (Row.provenance ~sh_id:33 ~kind:`Output) rho in
  let equality =
    Row.Row_eq
      { r1 = rho_row; r2 = closed_row ~sh_id:34 ~beg_dims:[ dim 3; dim 4 ] ~dims:[]; origin }
  in
  let inequality =
    Row.Row_ineq
      {
        res = closed_row ~sh_id:35 ~beg_dims:[ dim 3 ] ~dims:[ dim 2; dim 4 ];
        opnd = rho_row;
        origin;
      }
  in
  (rho, if eq_first then [ equality; inequality ] else [ inequality; equality ])

let () =
  let rho, constrs = prop_2_9_stage1_witness ~eq_first:true in
  check_stage1_defer "stage1 defers Prop 2.9 witness, equality first" rho constrs

let () =
  let rho, constrs = prop_2_9_stage1_witness ~eq_first:false in
  check_stage1_defer "stage1 defers Prop 2.9 witness, inequality first" rho constrs

let stage1_exact_binding ~sh_id ~beg_dims ~dims =
  let rho = Row.get_row_var () in
  let rho_row = Row.get_row_for_var (Row.provenance ~sh_id ~kind:`Output) rho in
  (rho, [ Row.Row_eq { r1 = rho_row; r2 = closed_row ~sh_id:(sh_id + 1) ~beg_dims ~dims; origin } ])

let () =
  let rho, constrs = stage1_exact_binding ~sh_id:37 ~beg_dims:[] ~dims:[] in
  check_stage1_defer "stage1 discharges empty row equality" rho constrs

let () =
  let rho, constrs = stage1_exact_binding ~sh_id:39 ~beg_dims:[] ~dims:[ dim 4 ] in
  check_stage1_defer "stage1 discharges isolated right-anchored row equality" rho constrs
