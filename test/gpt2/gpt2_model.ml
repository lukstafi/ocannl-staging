(* GPT-2 (decoder-only transformer, pre-LN, HuggingFace GPT2LMHeadModel semantics) assembled from
   ...
   e.g. the query projection is [n_heads; d_head; d_model] and maps from the HF Conv1D convention
   (y = x W + b with W of shape [d_in; d_out], fused qkv in c_attn) by transposition/slicing. *)
...
let hf_qkv_w name ~which ~n_heads ~d_head ~d_model st =
  ...
  permuted src
    ~dims:[ n_heads; d_head; d_model ]
    ~f:(fun idx -> [| idx.(2); off + (idx.(0) * d_head) + idx.(1) |])
...
(** Causal multi-head self-attention with projection biases (GPT-2 style). The sequence axis is
    the rightmost batch axis of [x]; [mask] is 1 where position [s] may attend to [t]. *)
let%op attention ~n_heads ~d_head () ~w_q ~b_q ~w_k ~b_k ~w_v ~b_v ~w_o ~b_o ~mask x =
  ...
  let attn_weights =
    Nb.softmax ~spec:" ... | t -> ..." () (where mask scores !.(-1e9))
  in
...
let block ~config ~src ~layer =
  ...
  let ln_1_g = w ~name:(p "ln_1.weight") ~center:1.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "ln_1.weight")) () in
  let ln_1_b = w ~name:(p "ln_1.bias") ~center:0.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "ln_1.bias")) () in
  let ln_2_g = w ~name:(p "ln_2.weight") ~center:1.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "ln_2.weight")) () in
  let ln_2_b = w ~name:(p "ln_2.bias") ~center:0.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "ln_2.bias")) () in
  let w_q = w ~name:(p "attn.w_q") ~b:[] ~o:[ n_heads; d_head ] ~i:[ d_model ] ~of_hf:(qkv_w 0) () in
  let w_k = w ~name:(p "attn.w_k") ~b:[] ~o:[ n_heads; d_head ] ~i:[ d_model ] ~of_hf:(qkv_w 1) () in
  let w_v = w ~name:(p "attn.w_v") ~b:[] ~o:[ n_heads; d_head ] ~i:[ d_model ] ~of_hf:(qkv_w 2) () in
  let b_q = w ~name:(p "attn.b_q") ~scale:0.0 ~b:[] ~o:[ n_heads; d_head ] ~i:[] ~of_hf:(qkv_b 0) () in
  let b_k = w ~name:(p "attn.b_k") ~scale:0.0 ~b:[] ~o:[ n_heads; d_head ] ~i:[] ~of_hf:(qkv_b 1) () in
  let b_v = w ~name:(p "attn.b_v") ~scale:0.0 ~b:[] ~o:[ n_heads; d_head ] ~i:[] ~of_hf:(qkv_b 2) () in
  let w_o =
    w ~name:(p "attn.c_proj.weight") ~b:[] ~o:[ d_model ] ~i:[ n_heads; d_head ]
      ...
      ()
  in
  let b_o = w ~name:(p "attn.c_proj.bias") ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "attn.c_proj.bias")) () in
  let w_fc = w ~name:(p "mlp.c_fc.weight") ~b:[] ~o:[ d_ff ] ~i:[ d_model ] ~of_hf:(hf_transpose (p "mlp.c_fc.weight")) () in
  let b_fc = w ~name:(p "mlp.c_fc.bias") ~scale:0.0 ~b:[] ~o:[ d_ff ] ~i:[] ~of_hf:(hf_id (p "mlp.c_fc.bias")) () in
  let w_proj = w ~name:(p "mlp.c_proj.weight") ~b:[] ~o:[ d_model ] ~i:[ d_ff ] ~of_hf:(hf_transpose (p "mlp.c_proj.weight")) () in
  let b_proj = w ~name:(p "mlp.c_proj.bias") ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id (p "mlp.c_proj.bias")) () in
  fun ~mask x ->
    let x =
      [%op
        x
    ...
    in
    [%op x + ffn () ~w_fc ~b_fc ~w_proj ~b_proj (layer_norm ~epsilon () ~gamma:ln_2_g ~beta:ln_2_b x)]
...
let gpt2 ~config ~src ~seq_len () =
  ...
  let lm_head = w ~name:"lm_head" ~b:[] ~o:[ vocab_size ] ~i:[ d_model ] ~of_hf:(hf_id "wte.weight") () in
  let pos_embed =
    w ~name:"wpe" ~b:[ seq_len ] ~o:[ d_model ] ~i:[]
      ...
      ()
  in
  let ln_f_g = w ~name:"ln_f.weight" ~center:1.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id "ln_f.weight") () in
  let ln_f_b = w ~name:"ln_f.bias" ~center:0.0 ~scale:0.0 ~b:[] ~o:[ d_model ] ~i:[] ~of_hf:(hf_id "ln_f.bias") () in
  ...
  let mask =
    NTDSL.init ~l:"causal_mask" ~prec:Ir.Ops.single ~b:[ seq_len ] ~i:[ seq_len ] ~o:[]
      ~f:(function
        ...
        | _ -> failwith "gpt2: unexpected mask indices")
      ()
  in
  fun ids ->
