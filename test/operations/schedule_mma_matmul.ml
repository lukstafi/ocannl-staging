...
let () =
  ...
  (* --- Transposed operand layouts (the gradient-GEMM access patterns): [d[i,j] += at[k,i] *
     ...
     operand copy, tolerance parity — the tile reduction reassociates); the C backends decline the
     register tiling on transposed layouts, so the scalar fallback keeps parity BITWISE. CUDA's wmma
     draft declines uniform f32 regardless. --- *)
    ...
    | Some src ->
        ...
        let ok =
          if on_metal then
            ...
            (* CUDA (uniform f32) and the C backends (transposed layouts) both decline to the scalar
               fallback; the register tiling must not fire on transposed layouts. *)
