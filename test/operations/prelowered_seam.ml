(* gh-ocannl-562: the [?prelowered] test seam — compiling and LINKING a hand-built
   ...
   case, unreachable through [Assignments] because the optimizer never writes a materialized node
   inside a scope body):

   [Y[i] = scopeA{ la := 0; for k: la := la + X[i] } + scopeB{ X[i] := 5; lb := X[i] * 2 }]

   The reads in [scopeA] precede the write in [scopeB], so the coverage query must NOT report X's
   reads as covered: X is read before written, hence a routine input whose incoming buffer contents
   are preserved. Under a wrong [`Covered] verdict X's placement stays undecided, the virtualizer
   inlines its trivial [5.0] setter into the scope reads, and the seeded values silently vanish —
   which shows up here as executed values, the leg structural pins cannot reach (verified by
   forcing the verdict during development: the value checks fail alongside the classification
   ones).
...
(* [Y[i] = scopeA{ la := 0; for k: la := la + X[i] } + scopeB{ X[i] := 5; lb := X[i] * 2 }] — X's
   placement is left to the pipeline, so the read-before-write classification is what keeps X a
   materialized input.

   [scopeA] is an inlined accumulation (the virtualizer's own shape for an inlined reduction) rather
   than the bare [la := X[i]] of the gh-ocannl-561 sketch: a single-assignment scope collapses to
   its value expression ([simplify_llc]), and scope bodies are HOISTED ahead of the enclosing
   statement, so the collapsed read would be ordered AFTER the sibling scope's write — unspecified
   territory for an IR shape the [Assignments] pipeline never emits (gh-ocannl-584). With a scope
   that survives, both readings agree: the scopes execute in scope-id order. *)
let phase2 () =
  ...
  (* Scope ids order the hoisted definitions in the generated code, so mint the reading scope
     first — the write in the sibling scope must not precede it. *)
    ...
    LL.Local_scope
      {
        id = id_b;
        body =
          seq (set i x (c 5.)) (LL.Set_local (id_b, binop Ops.Mul (get i x) (c 2.)));
        orig_indices = [| iter i |];
      }
  in
  ...
  match results with
  | [ yv; xv_out ] ->
      (* The sibling scope's reads see X's INCOMING value; the sibling write lands. *)
      p "phase2: Y = 2*X_in + 10 (the scope reads preserved the input)"
        (close yv (Array.map xv ~f:(fun v -> (2. *. v) +. 10.)));
      p "phase2: X was overwritten by the sibling scope" (close xv_out [| 5.; 5.; 5.; 5. |])
  | _ -> assert false
