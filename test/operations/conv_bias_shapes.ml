...
let print_params ~name t =
  ...
  |> List.map ~f:(fun p ->
         Printf.sprintf "  %s: %s" (Ir.Tnode.debug_name p.Tensor.value)
...
let () =
  ...
  let input = TDSL.range_of_shape ~label:[ "input" ] ~batch_dims:[ 2 ] ~output_dims:[ 8; 8; 1 ] () in
  ...
  (* Depthwise separable conv: pin the output channels to a single axis of dim 5 via a spec'd add
     of a fixed-shape tensor (a plain [+] would broadcast instead of pinning). *)
  let input2 =
