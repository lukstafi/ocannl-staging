(* Context.copy (docs/proposals/backend-singletons-context-copy.md): same-backend copies pair-match
   ...
   cross-backend copies fall back to a host round-trip. Backends are pinned explicitly (sync_cc /
   ...
   4. Cross-backend (sync_cc -> multicore_cc) host round-trip.
...
let scenario_cross_backend () =
  printf "=== 4. Cross-backend copy (sync_cc -> multicore_cc) ===\n";
  Tensor.unsafe_reinitialize ();
  let ctx = Context.cpu () in
