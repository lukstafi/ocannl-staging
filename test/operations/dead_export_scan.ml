(* gh-ocannl-806: a top-level helper in a module without an [.mli] is an exported value, even when
   ...
   counts every value because it re-exports the interface. Values generated by [sexp_of], [sexp],
   [compare], and [equal] derivings are exports too; the matching expression extensions count as
   references to the same generated names. Other PPX-generated values and values brought into the
...
let exempt_zero_reference_exports =
  [
    ...
    "PrintBox_utils.sexp_of_dag";
    "PrintBox_utils.sexp_of_table_row_spec";
