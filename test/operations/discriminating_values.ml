...
let () =
  ...
  p "cycle accepts the shapes the accumulator-width legs use"
    (List.for_all shapes ~f:(fun dims -> Option.is_none (Ll_test.blind_axis ~dims ~modulus:13)));
  p "drift varies with every index of every shape used"
    (List.for_all shapes ~f:(fun dims ->
  ...
  p "every drift cell is exact in bf16"
    (List.for_all shapes ~f:(fun dims ->
  ...
  p "a drift reduction leaves bf16 exactness within the extents these tests reduce"
    (List.for_all shapes ~f:(fun dims ->
         List.exists (partials ~dims) ~f:(fun s -> not (bf16_exact s))));
  p "every drift partial sum stays exact in f32"
    (List.for_all shapes ~f:(fun dims -> List.for_all (partials ~dims) ~f:f32_exact));
