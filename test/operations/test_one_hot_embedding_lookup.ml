...
(* --- AC 3: generated-C inspection for the large-index (> INT_MAX) path --- A real gather with
   ...
   is left at single/float32 (ids_wide.value.storage_prec = single → iprec = single → inner cast is (float)
...
let () =
  ...
  (* (1) Lower and scan IR to verify Get_dynamic.dyn_value carries iprec = double. This scan uses
     ...
     [Ir.Ops.single] before creating [ids_wide] would change [ids_wide.value.storage_prec] from [double] to
