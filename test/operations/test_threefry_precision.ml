...
let () =
  ...
  Stdlib.Printf.printf "Threefry4x32 result precision: %s\n"
    (Ir.Ops.prec_string (Lazy.force rng_result.value.storage_prec));
  ...
  Stdlib.Printf.printf "Uniform result precision: %s\n"
    (Ir.Ops.prec_string (Lazy.force uniform_result.value.storage_prec));
  ...
  try
    ...
    Stdlib.Printf.printf "Actual tensor precision in context: %s\n"
      (Ir.Ops.prec_string (Lazy.force tn.storage_prec))
  with
