(* OCANNL GPT-2-style inference runner: pre-LN decoder blocks built from
   Nn_blocks.multi_head_attention (fixture weights injected by name), with hand-built
   LayerNorm (nn_blocks.layer_norm currently subtracts an un-divided sum; see the pending fix
   task) and a tanh-gelu FFN from fixture-wrapped weight tensors. Token embedding is the
   logical one-hot gather (gh-343); the lm_head is tied to wte via an einsum that reads it
...
(* Correct LayerNorm (biased variance, epsilon inside the sqrt), mirrored exactly by the
   Python runners. *)
let%op lnorm ~g ~b x =
  let mu = (x ++ " ... | ..d.. => ... | 0 " [ "d" ]) /. dim d in
  let c = x - mu in
  let v = ((c *. c) ++ " ... | ... => ... | 0 ") /. dim d in
  ((c /. sqrt (v + !.1e-5)) *. g) + b
    ...
    List.init n_layer ~f:(fun i ->
        ...
        let mha =
          Nn_blocks.multi_head_attention
            ...
            ~num_heads:n_head ~d_k:d_head ~d_v:d_head ()
        in
        let g1 = wrap (name "l%d_ln1_g") ~i:[] ~o:[ d_model ] in
        let b1 = wrap (name "l%d_ln1_b") ~i:[] ~o:[ d_model ] in
        let g2 = wrap (name "l%d_ln2_g") ~i:[] ~o:[ d_model ] in
        let b2 = wrap (name "l%d_ln2_b") ~i:[] ~o:[ d_model ] in
        ...
        fun x ->
          let%op x1 = x + mha ~train_step:None ~mask (lnorm ~g:g1 ~b:b1 x) in
          let%op x2 = x1 + ((fw2 * gelu ((fw1 * lnorm ~g:g2 ~b:b2 x1) + fb1)) + fb2) in
          x2)
  in
  let hfinal =
    lnorm
      ~g:(wrap "lnf_g" ~i:[] ~o:[ d_model ])
      ~b:(wrap "lnf_b" ~i:[] ~o:[ d_model ])
      (List.fold layers ~init:embedded ~f:(fun x layer -> layer x))
  in
