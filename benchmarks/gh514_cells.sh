...
# always run at f32 — at reduced precision the training step is loss-scale-gated and
# Bench_harness.compile_train_step's gated arms bypass the model_default gate, so a reduced-
# precision D comparison would measure identical executions (see report-gh514-eval.md, the
# harness-gap note). Control cells pin their experimental gates to their disabled values
...
mlp A 1 "$PREC" --ocannl_autotune_bound_pruning=false \
...
mlp B-off 1 "$PREC" ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_autotune_bound_pruning=false
mlp B-on 1 "$PREC" ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_autotune_bound_pruning=true
# C: the flip chain at budget 5, legacy cost ordering vs enablement ordering, two replicates;
# then enablement + bound pruning.
for r in 1 2; do
  mlp C-cost-$r 1 "$PREC" ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_autotune_bound_pruning=false \
    --ocannl_tune_inline_flips=5 --ocannl_tune_flip_ordering=cost
  mlp C-enab-$r 1 "$PREC" ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_autotune_bound_pruning=false \
    --ocannl_tune_inline_flips=5 --ocannl_tune_flip_ordering=enablement
done
mlp C-enab-bp 1 "$PREC" ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_tune_inline_flips=5 \
  --ocannl_tune_flip_ordering=enablement --ocannl_autotune_bound_pruning=true
# D: the untuned regime — default pipeline vs model_default, +placements, +lattice. Always f32
# (see the header note); each arm pins the gates the treatment does not enable.
mlp D-default 0 f32 --ocannl_model_default_schedule=false
mlp D-model 0 f32 ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_model_default_schedule=true \
  --ocannl_model_default_placements=0 --ocannl_model_default_geometry_lattice=false
mlp D-model-plc 0 f32 ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_model_default_schedule=true \
  --ocannl_model_default_placements=5 --ocannl_model_default_geometry_lattice=false
mlp D-model-lat 0 f32 ${PEAKS[@]+"${PEAKS[@]}"} --ocannl_model_default_schedule=true \
