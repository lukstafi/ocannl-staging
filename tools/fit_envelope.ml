(* Fit per-machine roofline envelope constants (config [model_peak_flops] /
   ...
   measured (fitted peaks are floors, not certified maxima), at the cost of proportionally looser
   bounds. Files from several tuning runs on the same machine can be concatenated or passed together
   — the fit only tightens with more rows.
...
let () =
  ...
  List.iter args ~f:(fun a ->
      ...
      | None -> (
          ...
          | Some m -> (
              ...
              | _ | (exception _) ->
                  (* Below 1 the "headroom" would shrink the peaks — reintroducing exactly the bound
                     violations the fit just removed. *)
                  Stdio.eprintf "fit_envelope: --margin needs a float >= 1.0, got %s\n" m;
                  Stdlib.exit 2)
          | None ->
              (* A known config key under any accepted spelling belongs to the config machinery (see
                 [Utils.cmdline_arg_is_config_key]); an unrecognized dash argument is a probable
                 typo. *)
            ...
            if String.is_empty (String.strip line) then None
              ...
              | None ->
                  (* Rows recorded before the approx column (the original 9-column schema of
                     gh-ocannl-491) cannot prove their counts exact, so they cannot enter the fit —
                     but dropping them as generic garbage would hide that the file's fastest
                     candidates may be missing. Name them explicitly. *)
  ...
  if !legacy > 0 then
    Stdio.eprintf
      "fit_envelope: skipped %d legacy 9-column row(s) (recorded before the approx-count column): \
       they cannot prove exact counts, so the fit may miss the fastest previously observed \
       candidates — prefer re-recording calibration data with the current build\n"
      !legacy;
  ...
  let fits =
    ...
    | Some b -> (
        ...
        | selected -> selected)
  in
  (* The model_peak_* keys are global, so concatenated per-backend sections repeat them and are not
     pasteable into one config file. *)
  if List.length fits > 1 then
    Stdio.eprintf
      "fit_envelope: %d backends in the data (%s) — sections below repeat the global model_peak_* \
       keys; pass --backend=NAME for a config-pasteable single section\n"
      (List.length fits)
      (String.concat ~sep:", " (List.map fits ~f:(fun f -> f.Cal.fit_backend)));
  let with_margin f =
