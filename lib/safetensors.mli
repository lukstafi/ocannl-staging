...
(** [to_float32 t name] reads the named tensor's payload from the file into a fresh float32 Genarray
    of the tensor's shape (a scalar shape becomes a 1-element 1-D array).

    Raises [Failure] if the tensor is missing or its dtype is not "F32". *)

val to_ndarray : t -> string -> Ir.Ndarray.t
(** [to_ndarray t name] is {!to_float32} wrapped as an OCANNL single-precision ndarray, suitable for
    tensor [init_data] (e.g. [TDSL.wrap] / [TDSL.rebatch]). *)
