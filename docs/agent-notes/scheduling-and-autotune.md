...
- **What the tuner's numbers are a measurement OF is a config choice, and the two objectives do not
  ...
  up to four further batch probes; each short probe refits the affine model. After four noisy misses
  the latest projected depth wins rather than jumping to a 20--30 ms cap batch, because such an
  overlong batch would blunt the 2x contention threshold. On Metal's ~0.17 ms kernels the first
  probe already spans the target and the historical ~59 depth is unchanged; on faster CUDA/HIP
  kernels it grows the batch toward the same wall target.
