...
- `dune build @bin-smoke` runs every `bin/` executable sequentially on deliberately tiny workloads
  ...
  Alias edges and generated file producers in a public executable's `link_deps` are also traversed
  and refused if they run a public executable or contain an opaque dependency/action: they execute
  while the smoke target is being built, before its credited runtime canary.
