...
* **Stage 1** is online as tensors are composed, and conservatively performs unification and constraint propagation. The important exception is open/closed `Row_eq` whose binding would be non-empty and whose row variable also appears in a row inequality in the same worklist: stage 1 substitutes facts already known in the environment and rejects necessary failures, but re-emits the equality instead of binding the row variable to that inherited marker split. Empty closures, isolated exact bindings, and internal rank-growth row equalities emitted by inequality solving still discharge in stage 1. Stages 2–7 are only performed once necessary: when projections or dimensions are requested.
