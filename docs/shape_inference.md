...
Shape inference broadly speaking consists in OCANNL of inferring the `Shape.t` record -- shape inference proper, and inferring the `Indexing.projections` record -- projections inference. `Shape.t` records are mutable, so that the partially inferred shapes can be observed by the user. Shape and projections inference is intended to be as declarative as practical -- independent of incidental constraint-generation order during the online pass. The main exceptions are deliberate closing and placement policies: when tensor expressions are compiled to assignments, i.e. jitted, still-unsolved shape variables in terminal nodes are substituted by their greatest lower bounds if any, or by dimension-1 (no basis) / no-more-axes; and ambiguous flat row equalities choose a concrete marker placement only after the online stage has accumulated the surrounding row constraints.
...
The actual implementation is split into the `Row` module, which handles multi-row inference, and the `Shape` module which deals with the specific axis kinds (batch, input, output), _einsum_ specifications, and the shape-relevant semantics of operations expressed via the `Shape.logic` variant type. A row is represented by an outer-left flank (`beg_dims`), an outer-right flank (`dims`), and a middle marker (`bcast`). Substituting a `row_var` therefore fills the middle between the two flanks, not merely a prefix or suffix. For row equality, closed rows are compared by flattened content (`beg_dims @ dims`), while row inequalities remain marker-sensitive.
...
|:-----:|------|----------|----------|
| **1** | Online | Unify, propagate | Partial unify, propagate |
| **2** | On demand | Force precision coefficients | Full unify |
...
* **Stage 1** is online as tensors are composed, and conservatively performs unification and constraint propagation. The important exception is open/closed `Row_eq` whose binding would be non-empty and whose row variable also appears in a row inequality in the same worklist: stage 1 substitutes facts already known in the environment and rejects necessary failures, but re-emits the equality instead of binding the row variable to that inherited marker split. Empty closures, exact bindings with no same-worklist marker-sensitive observer, and internal rank-growth row equalities emitted by inequality solving still discharge in stage 1. Stages 2–7 are only performed once necessary: when projections or dimensions are requested.
...
* `unify_row`: solves a single equation between two rows, and recursively all `dim` and `row` equations that this entails, but not other constraints.
  ...
  * `solve_inequalities` deliberately does not call it in stage 1 for open/closed row equality whose binding would be non-empty when the same worklist contains a row inequality mentioning the bound variable. Such an equality is checked by `check_row_eq_stage1` and left in the worklist until stage 2, so the inherited-split marker policy is not applied merely because the equality happened to be generated before a related row inequality.
