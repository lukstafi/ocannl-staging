...
\title{Neural Networks Without Shape Boilerplate: An OCaml DSL Case Study}
...
with momentum and Nesterov-inspired correction. OCANNL performs broadcast-aware global
...
shape inference problem is non-principal, and the
...
down for brevity) and GPT 5.5 (fixing issues I noticed, final compilation and proof gaps).
...
always satisfiable; \(\gamma_\uparrow\) is its greatest solution).} Define \(\gamma_\uparrow\) on the unsolved variables:
...
in this case. TR Example 3.6 shows the restriction
is necessary.
...
\item
  ...
  sort: row equations --- including
  \(\mathrm{eqns}(\sigma_\star)\) --- are read under \(\approx\), so a
  solution may re-place the marker of a solved row variable, and
  distinct placements are pairwise \(\sqsubseteq\)-incomparable (TR
  Proposition 2.9(ii)); once a row variable is solved to a row of
  positive rank, no \(\sqsubseteq\)-greatest solution exists at all.
  The solver's substitution commits one placement --- a policy
  incompleteness (TR Lemma 5.1), not order-theoretic strength.
\item
