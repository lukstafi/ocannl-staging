...
| [build-and-test.md](agent-notes/build-and-test.md) | Dune mechanics behind the workflow rules: scanning checks, `PIN THE RELATIONSHIP, NOT THE RESTATEMENT`, `agent_notes_structure` over these very files, `tools/test-run.sh`, `copy_files` and env-var tracking, `@check` not linking, worktree roots, plus what CI actually covers. |
