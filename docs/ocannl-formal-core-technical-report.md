...
Operationally, stage 1 now avoids this commitment when the same stage-1
worklist contains an inequality that can observe the placement. If an
open/closed row equality would bind the row variable to a non-empty middle
segment and a row inequality in the same worklist mentions that variable,
stage 1 treats the equality as a constraint, not as a rewrite: it substitutes
...
The stage-1 implementation refinement does not change this proof. Delaying an
open/closed row equality whose binding would be non-empty is a semantic
no-op: the same constraint remains in `Phi`. The online check only rejects
cases already shown unsatisfiable by Lemma 4.5. When the equality is deferred,
the later stage-2 discharge is the same policy-refining RE-open-closed step
accounted for above.
...
Earlier solver versions could make this commitment during stage 1, so merely
prepending or appending constraints could determine whether the row equality
was substituted before the row inequality had recorded its cap. The current
implementation removes that online-order dependence for the same-batch form
of the witness: because the worklist also contains a row inequality mentioning
`rho`, stage 1 records and checks the equality but postpones the
marker-placement rewrite to stage 2. This is not a completeness theorem for
the mixed flat/marked theory; it only makes the first policy point
phase-delimited when a marker-sensitive obligation is locally visible, rather
than an accident of constraint order inside that generated batch.
